#!/usr/bin/env python3
"""Offline fixed-pose XY alignment audit from a saved, paired state/vision sample.
...
from manual_motor_probe_guard import MEASURED_JOINT_SPEED_LIMIT_DEG_S, MEASURED_TCP_SPEED_LIMIT_MM_S
...
def plan(snapshot_path, output, duration=30.):
    if not np.isfinite(duration) or not 15 <= duration <= 60:
        raise ValueError('Diagnostic duration must be 15--60 seconds')
    data = json.loads(snapshot_path.read_text())
    q0, target_xy = validate_snapshot(data)
    ...
    report = {'utc': datetime.now(timezone.utc).isoformat(), 'mode': 'offline_fixed_pose_XY_alignment',
              ...
              'remaining_blockers': ['specific_motion_not_authorized', 'actual_tool_envelope_supports_cables_and_people_not_verified',
                                     ...
                                     'live_XY_executor_not_yet_reviewed', 'H_response_to_static_magnet_translation_unknown',
    ...
    try:
        ...
        phase = timed_phase(knots, 1., duration)
        points = [dict(phase[0], time_s=0.)]+phase+[dict(phase[-1], time_s=duration+3.)]
        ...
        draft = {'execution_allowed': False, 'joint_names': JOINTS, 'sample_period_s': .008, 'points': points,
                 'scope': 'one fixed-pose XY alignment only; no square, no motor operation',
                 'phases': {'move': [1., duration+1.], 'final_hold': [duration+1., duration+3.]}}
        ...
        # Independent measured-speed stopping sensitivity. This is deliberately
        # separate from the lower planned speed and uses the operator-approved
        # measured joint abort threshold.
        w = np.deg2rad(MEASURED_JOINT_SPEED_LIMIT_DEG_S)
        perturb = .0015+w*.2+w*w/2
        measured_stop_states = []
        measured_stop_minima = dict.fromkeys(BOUNDARIES, float('inf'))
        for joint in np.asarray(knots)[::5]:
            for signs in itertools.product((-1, 1), repeat=6):
                stopped = joint+perturb*np.asarray(signs)
                bounds, _ = geometry.bounds(stopped)
                for key, value in bounds.items():
                    measured_stop_minima[key] = min(measured_stop_minima[key], value*1000)
                measured_stop_states.append(stopped)
        measured_stop_fcl = check_collision(
            binaries/'check_self_collision', urdf, srdf, padded, measured_stop_states,
            output/'measured_speed_stop_fcl.csv',
            [geometry.model.transforms(dict(zip(JOINTS, q)))['tool0'] for q in measured_stop_states])
        measured_limits = clearance_limits_mm(ROOT)
        measured_stop_passed = all(measured_stop_minima[k] >= v for k, v in measured_limits.items())
        if not measured_stop_passed:
            raise ValueError('Measured-speed stopping sensitivity clearance failed')
        report.update(checks_pass_for_draft=True, sphere_start_world_mm=(geometry.start_sphere*1000).tolist(),
                      ...
                      reviewed_measured_joint_abort_deg_s=MEASURED_JOINT_SPEED_LIMIT_DEG_S,
                      reviewed_measured_tcp_abort_mm_s=MEASURED_TCP_SPEED_LIMIT_MM_S,
                      measured_speed_stop_sensitivity_passed=measured_stop_passed,
                      measured_speed_stop_samples=len(measured_stop_states),
                      measured_speed_stop_min_gaps_mm=measured_stop_minima,
                      measured_speed_stop_fcl=measured_stop_fcl,
                      measured_speed_stop_model={'latency_s': .2, 'deceleration_rad_s2': 1.,
                          'initial_per_joint_tracking_error_rad': .0015, 'physically_validated': False,
                          'method': 'independent per-joint box sensitivity'},
