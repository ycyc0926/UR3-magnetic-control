"""Local browser preview for the vision-only H tracker."""
...
#trajectory{cursor:default;background:white;margin-top:10px}#trajectory-wrap{display:none;margin-top:20px}
...
let rec=s.recording||{},m=rec.motion||{};
if(rec.active){record.textContent='● 正在录像 '+rec.frames+' 帧 / 剩余 '+Math.ceil(rec.seconds_remaining)+' 秒';record.className='value ok';motion.textContent='录像完成后自动分析中心轨迹。';}
else if(rec.classification==='moving'){record.textContent='● 录像已保留 · '+(rec.frames||0)+' 帧';record.className='value ok';motion.textContent='中心活动范围 '+(m.center_span_mm||0).toFixed(3)+' mm（阈值 '+(m.movement_threshold_mm||0).toFixed(3)+' mm）。';}
else if(rec.classification==='static'){record.textContent='● 静止录像已筛除，未保存';record.className='value';motion.textContent='中心活动范围 '+(m.center_span_mm||0).toFixed(3)+' mm，未达 '+(m.movement_threshold_mm||0).toFixed(3)+' mm 阈值。';}
else if(rec.classification==='indeterminate'){record.textContent='● 录像已保留（跟踪数据不足）';record.className='value bad';motion.textContent='无法安全判定是否静止，因此未自动删除。';}
else{record.textContent=rec.error?'录像错误：'+rec.error:'录像未启动';record.className='value bad';}
            ...
            def do_GET(self):
                if self.path == '/':
                    ...
                    try:
                        ...
                        pass
                elif self.path.startswith('/trajectory') and owner.trajectory is not None:
                    self.reply(owner.trajectory, 'image/png')
                else:
                    self.send_error(404)
